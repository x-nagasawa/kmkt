...
import com.github.kmkt.util.ImageServlet;
    ...
    public static void main(String[] args) {
        try {
            if (args.length < 2) {
                ...
                System.out.println();
                System.out.println("USAGE A : ImageServletTest [-p <port>] (duration) (jpg) (jpg) ...");
                System.out.println("USAGE B : ImageServletTest [-p <port>] (duration) (lst)");
            ...
            String imagepath = DEFAULT_IMAGEPATH;
            ...
            if (f.exists() && f.isFile()) {
                ...
                try {
                    ...
                    if (serverprop.containsKey("path")) {
                        imagepath = serverprop.getProperty("path");
                    }
                } catch (IOException e) {
            ...
            Thread t = new Thread(new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        while (true) {
                            servlet.pourFrame(frames.get(frame), "image/jpeg");   // Servlet 側でフレームが取り出されるまでブロック
