 ...
 * MjpegServlet 出力サンプル
 * 
 * USAGE: MjpegServerTest (wait) (jpg) (jpg) ...
 * wait : フレーム間ウエイト (ms)
...
public class MjpegServerTest {
    ...
    private static final String DEFAULT_WEBCAMPATH = "/mjpegfeed";
    ...
    public static void main(String[] args) {
        try {
            if (args.length < 2) {
                System.out.println("MJPEG 出力サンプルサーバ");
                System.out.println();
                System.out.println("USAGE A : MjpegServerTest [-p <port>] (duration) (jpg) (jpg) ...");
                System.out.println("USAGE B : MjpegServerTest [-p <port>] (duration) (lst)");
                System.out.println("port     : ポート番号");
                System.out.println("duration : フレーム間隔 (ms)");
                System.out.println("jpg      : MJPEG の1フレームとして送出するJPEGファイル 複数指定時は先頭から順に送出される");
                System.out.println("lst      : MJPEG の1フレームとして送出するJPEGファイルのリスト");
                System.exit(-1);
            }
            
            // デフォルト値
            int webport = DEFAULT_WEBPORT;
            String webcampath = DEFAULT_WEBCAMPATH;
            ...
            if (f.exists() && f.isFile()) {
                ...
                try {
                    ...
                    if (serverprop.containsKey("path")) {
                        webcampath = serverprop.getProperty("path");
                    }
                } catch (IOException e) {
            ...
            // MJPEGのセットアップ
            final MjpegServlet servlet = new MjpegServlet();
            ServletHolder wsh = new ServletHolder(servlet);
            root.addServlet(wsh, webcampath);
            ...
            System.out.println("Webcam path              : " + webcampath);
